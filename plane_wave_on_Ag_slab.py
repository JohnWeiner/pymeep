...
geometry = [mp.Block(size=mp.Vector3(w,mp.inf,mp.inf),
                     center=mp.Vector3(0,0,0),
                     material=Ag)]

wvlencen=0.532 #wavelength = 532nm
fcen=1/wvlencen
fcen=0.15  #pulse center frequency
df = 0.1*fcen      #pulse width

sources=[mp.Source(mp.GaussianSource(fcen, fwidth=df),
        component=mp.Ez,
        center=mp.Vector3(-0.5*sx+1,0,0),
...
refl_fr=mp.FluxRegion(center=mp.Vector3(-0.5*sx+dpml+0.5,0,0),
...
pt=mp.Vector3(-0.5*sx+1,0,0) #point for monitoring the field intensity of Gaussian source
